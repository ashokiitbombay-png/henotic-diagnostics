          ...
          {services.map((s) => (
            <Link key={s.id} href={s.link} className="group relative bg-slate-900 rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 h-[350px]">
              <div className="absolute inset-0">
                <img decoding="async" src={s.img} alt={s.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 opacity-70 group-hover:opacity-40" fetchPriority="high" />
